...
<?php
  $total=0;
	foreach($_SESSION['cart'] as $pd_id=>$o_qty)
	{
		$sql	= "select * from products where pd_id=$pd_id";
		$query	= mysqli_query($objCon, $sql);
		$row	= mysqli_fetch_array($query);
		$sum	= $row['pd_price']*$o_qty;
    $total	+= $sum;
    //echo "<input type='hidden' name='o_qty'>
          //<input type='hidden' name='o_qty'>";
    echo "<tr>";
    echo "<td>" . $row["pd_name"] . "</td>";
    echo "<td align='right'>" .number_format($row['pd_price'],2) ."</td>";
    echo "<td align='right'>$o_qty</td>";
...
<tr>
    ...
    <td><input name="o_name" type="text" id="o_name" required/></td>
</tr>
<tr>
    <td width="22%" bgcolor="#EEEEEE">ที่อยู่</td>
    <td width="78%">
    <textarea name="o_addr" cols="35" rows="5" id="o_addr" required></textarea>
    </td>
</tr>
<tr>
  	<td bgcolor="#EEEEEE">อีเมล</td>
  	<td><input name="o_email" type="email" id="o_email"  required/></td>
</tr>
<tr>
  	<td bgcolor="#EEEEEE">เบอร์ติดต่อ</td>
  	<td><input name="o_phone" type="text" id="o_phone" required /></td>
</tr>
<tr>
